...
test.describe('Сложные случаи для getByPlaceholder()', () => {
  ...
  test('Найти textarea по многострочному placeholder', async ({ page }) => {
    const textarea = page.getByPlaceholder(/Введите.*Можно/s); // TODO(student): замените на корректный локатор
      await expect(textarea).toBeVisible();
  });

  // Задание 2: Найди поле с пробелами в placeholder
  test('Найти поле с пробелами в placeholder', async ({ page }) => {
    const spacedInput = page.getByPlaceholder(/^\s|\s$/); // TODO(student): замените на корректный локатор
  ...
  test('Работа с динамическими полями', async ({ page }) => {
    const dynamicInput = page.getByPlaceholder('Динамическое'); // TODO(student): замените на корректный локатор
